...
// Импорт МультиВыбора: пример на shadcn/radix
...
export function CreateTicketDialog({
  ...
  // Для правильного связывания MultipleSelector с Service[] (formData.services)
  const serviceOptions = Array.from(services).map((service) => ({
    value: service.id,
    label: service.name,
  }));

  // Получить Option[] из services для value пропса MultipleSelector
  const selectedServiceOptions = formData.services.map((service) => ({
    value: service.id,
    label: service.name,
  }));

  // Хендлер правильно добавляет и удаляет сервисы в formData.services по selected в MultipleSelector
  const handleServicesChange = (
    selected: { value: string; label: string }[],
  ) => {
    // Получить Service[] по selected Option[]
  ...
  return (
    <DialogContent className=" border-border/50 w-[calc(100%-2rem)] sm:max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Новое поле для выбора сервисов */}
        <div className="space-y-2">
